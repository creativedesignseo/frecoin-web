...
4. **Config files unchanged or intentionally changed.**
   - Check the diff for hosting config (`netlify.toml`, `.netlify/`,
     the deploy workflow), `public/send-form.php`, and any other
...
- Read-only platform status commands (`netlify status`,
  `netlify deploy --dry-run` where supported, etc.)
